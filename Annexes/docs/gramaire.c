...
Rules 1 : 
	...
	Note:
		...
		so escaped <newline>s are already removed at this point.
...
Rules 9 :
	...
	Each TOKEN that might either be expanded or have assignment applied to it shall instead be returned as a single WORD consisting only of characters that are exactly the token described in Token Recognition.
